...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fine-tune the manual ECGFounder on PTB-XL Super5."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    plan = {
        ...
        "pretrained_checkpoint_path": (
            None if pretrained is None else str(Path(pretrained).resolve())
        ),
        "task_checkpoint_path": (
            None if task_checkpoint is None else str(Path(task_checkpoint).resolve())
        ),
    ...
    model = build_model(
        ...
        task_checkpoint_path=task_checkpoint,
        trainable_scope=trainable_scope,
        map_location="cpu",
    )
    result = train_ptbxl(
