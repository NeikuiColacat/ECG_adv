#!/bin/bash
# Synth-anchored online AT pilot — Iteration 2.
# Change: adv_weight 2.0 → 0.5 (subagent H3, Wang 2023 ICML lower synth ratio).
# Goal: break through +0.30/+0.50pp decision gate.
set -euo pipefail
...
OUT_BASE=${ROOT}/synth_anchored_super5/runs_iter2
mkdir -p "${OUT_BASE}"
...
run_cell() {
    ...
    echo
    echo "============================================================"
    echo "[pilot-iter2] cell: ${CENTER} (tag=${TAG}_k200)  adv_weight=0.5"
    echo "============================================================"
    cd "${REPO}"
...
echo
echo "============================================================"
echo "[pilot-iter2] all cells complete — running aggregator"
echo "============================================================"
"${PYBIN}" "${REPO}/scripts/pgd_cross_center/aggregate_pilot_results.py" \
    --runs_dir "${OUT_BASE}" \
    --tags extra_k200 nin_k200 \
    --out "${REPO}/docs/synth_anchored_super5_pilot_iter2.md"
echo "[pilot-iter2] done."
