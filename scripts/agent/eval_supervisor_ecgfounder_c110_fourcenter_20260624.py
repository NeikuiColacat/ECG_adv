...
ROOT = Path(
    ...
    "pn2021c_goal_ecgfounder_officials5_c110_20260624/"
    "eval_c110_officials5_fourcenter"
)
LOGDIR = Path("/dev/shm/ecg_goal_20260624_ecgfounder_c110")
DIRECT_ROOT = Path(
    "/home/linbinhao/ECG_adv_data/runs/"
    "pn2021c_official_s5_composite_direct_drop_20260622/ecgfounder"
)
ECGFOUNDER_CHECKPOINT = Path(
    "/home/linbinhao/ECG_adv_data/ecgfounder/checkpoint/12_lead_ECGFounder.pth"
)
...
class EvalTask:
    ...
    def out_json(self) -> Path:
        return self.out_dir / "eval_official_s5_locked_composite_bottleneck5000.json"

    @property
    def log_path(self) -> Path:
        return LOGDIR / f"ecgfounder_c110_{self.center}_eval_watch.log"

    def model_dir(self) -> Path | None:
        if not self.family_root.exists():
            return None
        candidates = sorted(
            p / "runs" / self.run_leaf
            for p in self.family_root.glob("goal_ecgfounder_c110*")
            if (p / "runs" / self.run_leaf).is_dir()
        )
        if not candidates:
            candidates = sorted(
                p / "runs" / self.run_leaf
                for p in self.family_root.iterdir()
                if p.is_dir() and (p / "runs" / self.run_leaf).is_dir()
            )
...
TASKS = [
    EvalTask(
        ...
        Path(
            "/home/linbinhao/ECG_adv_data/runs/"
            "ecgfounder_vae_lhat_augmix_threechain_officials5_cand110_weakcombo_cover_c24_cpsc_20260624"
        ),
        "cpsc_2018_vae_lhat_augmix_threechain_officials5_cand110_weakcombo_cover_c24",
    ),
    EvalTask(
        "georgia",
        1,
        Path(
            "/home/linbinhao/ECG_adv_data/runs/"
            "ecgfounder_vae_lhat_augmix_threechain_officials5_cand110_weakcombo_cover_c24_georgia_20260624"
        ),
        "georgia_vae_lhat_augmix_threechain_officials5_cand110_weakcombo_cover_c24",
    ),
    EvalTask(
        "chapman_shaoxing",
        2,
        Path(
            "/home/linbinhao/ECG_adv_data/runs/"
            "ecgfounder_vae_lhat_augmix_threechain_officials5_cand110_weakcombo_cover_c24_chapman_shaoxing_20260624"
        ),
        "chapman_shaoxing_vae_lhat_augmix_threechain_officials5_cand110_weakcombo_cover_c24",
    ),
    EvalTask(
        "ningbo",
        3,
        Path(
            "/home/linbinhao/ECG_adv_data/runs/"
            "ecgfounder_vae_lhat_augmix_threechain_officials5_cand110_weakcombo_cover_c24_ningbo_20260624"
        ),
        "ningbo_vae_lhat_augmix_threechain_officials5_cand110_weakcombo_cover_c24",
    ),
]
...
def eval_command(task: EvalTask, *, model_dir: Path) -> list[str]:
    return [
        str(PY),
        "-u",
        ...
        str(model_dir),
        "--variant",
        "c110_officials5_weakcombo_cover",
        "--checkpoint",
        str(ECGFOUNDER_CHECKPOINT),
        "--corruption_input",
        "bottleneck5000",
        "--pn2021_root",
...
def summary_command() -> list[str]:
    ...
    for center in ["chapman_shaoxing", "cpsc_2018", "georgia", "ningbo"]:
        cmd.extend(
            [
                "--direct-json",
                f"{center}={DIRECT_ROOT / center / 'eval_official_s5_composite_direct_bottleneck5000.json'}",
            ]
        )
    for task in TASKS:
        cmd.extend(["--method-json", f"{task.center}={task.out_json}"])
    cmd.extend(
        [
            "--output-dir",
            str(ROOT / "summary"),
            "--prefix",
            "ecgfounder_c110_fourcenter_official_s5_composite_method",
        ]
    )
    return cmd
...
def main() -> int:
    ...
    master_log = LOGDIR / "eval_supervisor_ecgfounder_c110_fourcenter_20260624.log"
    ...
    while True:
        for task in TASKS:
            ...
            if model_dir is None:
                continue
            if not (model_dir / "last_model.pt").exists() or not (model_dir / "eval_result.json").exists():
                continue
            try:
            ...
            env.update(
                {
                    ...
                    "TORCH_NUM_THREADS": "6",
                    "PYTHONUNBUFFERED": "1",
                    "ECGFOUNDER_ROOT": "/home/linbinhao/ECG_adv_data/ecgfounder",
                    "ECG_ADV_GEN_DATA_ROOT": "/home/linbinhao/ECG_adv_data",
                }
            )
            log_f = task.log_path.open("a", encoding="utf-8")
            append(task.log_path, f"eval_start key={key} gpu={task.gpu} run_dir={model_dir}")
            running[key] = subprocess.Popen(
                eval_command(task, model_dir=model_dir),
                cwd=REPO,
                env=env,
                stdout=log_f,
        ...
        if len(complete) == len(TASKS) and not summary_done:
            log_path = LOGDIR / "ecgfounder_c110_fourcenter_summary_watch.log"
