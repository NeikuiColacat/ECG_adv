#!/usr/bin/env python3
"""DANN K=500 target-center adaptation baseline for Super5 PN2021.

This script reuses the same data/evaluation helpers as the Deep CORAL runner
and imports THUML Transfer-Learning-Library's GRL and DomainDiscriminator
modules directly from the cloned repo. The domain-adversarial loss follows the
official DANN formula, but is kept local to avoid optional package dependencies
pulled in by tllib.__init__.
"""
...
from scripts.paper.run_deep_coral_k500_20260517 import (  # noqa: E402
    BASELINE_CKPT,
    CLASS_NAMES_SUPER5,
    DEFAULT_OUT_ROOT as CORAL_OUT_ROOT,
    NUM_SUPER5,
    PN2021_CACHE_DIR,
...
DEFAULT_OUT_ROOT = Path("/root/autodl-tmp/paper_uda_baselines_20260517/dann_k500")
TLL_ROOT = Path("/root/autodl-tmp/external_repos/Transfer-Learning-Library")
GRL_FILE = TLL_ROOT / "tllib/modules/grl.py"
DOMAIN_DISC_FILE = TLL_ROOT / "tllib/modules/domain_discriminator.py"


def load_class_from_file(path: Path, class_name: str):
    if not hasattr(np, "float"):
        np.float = float  # type: ignore[attr-defined]
    spec = importlib.util.spec_from_file_location(f"official_{path.stem}_{class_name}", path)
...
def dann_tag(args: argparse.Namespace) -> str:
    return f"dann_lam{args.dann_lambda:g}_h{args.domain_hidden}".replace(".", "p")
...
def domain_adversarial_loss(
    ...
    f = grl(torch.cat((f_s, f_t), dim=0))
    d = discriminator(f).float()
...
def train_one(center: str, args: argparse.Namespace) -> Path:
    ...
    print(
        ...
        f"dann_lambda={args.dann_lambda}",
        flush=True,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x_s, y_s in source_loader:
            ...
            loss_domain, domain_acc = domain_adversarial_loss(feat_s, feat_t, discriminator, grl)
            loss = loss_cls + float(args.dann_lambda) * loss_domain
...
def parse_eval(center: str, args: argparse.Namespace, eval_path: Path) -> dict:
    ...
    return {
        ...
        "dann_lambda": float(args.dann_lambda),
...
def write_summary(rows: list[dict], out_root: Path) -> None:
    ...
    csv_path = summary_dir / "dann_k500.csv"
    fields = [
        "method",
        "tag",
        "center",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dann_lambda", type=float, default=0.1)
...
def main() -> None:
    ...
    for center in args.centers:
        ...
        if not args.dry_run:
            rows = [r for r in rows if not (r["center"] == center and r["tag"] == dann_tag(args))]
