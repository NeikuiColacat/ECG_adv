...
from ecg_adv_gen.adaptation import build_k500_internal_val_mask  # noqa: E402
from ecg_adv_gen.evaluation.pn2021_corruptions import stable_corruption_seed  # noqa: E402
from scripts.triple_labels.eval_crosscenter import infer_dataset  # noqa: E402
...
def _signals_tc_to_ct(signals_tc: np.ndarray, crop_len: int) -> np.ndarray:
    signals_ct = np.transpose(signals_tc, (0, 2, 1)).astype(np.float32)
    start = max((signals_ct.shape[-1] - int(crop_len)) // 2, 0)
    return signals_ct[..., start : start + int(crop_len)]


def _has_input_stabilizer(config: dict[str, Any] | None) -> bool:
...
def evaluate_candidate(
    ...
    *,
    ...
    signals_ct_raw = _signals_tc_to_ct(signals_tc, crop_len)
    ...
    for op_name in corruptions:
        ...
        corrupted_ct = _apply_input_stabilizer_batch(corrupted_ct, input_stabilizer_config)
...
def run_selection(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "selection_rule": {
            ...
            "input_stabilizer": (
                input_stabilizer_config if _has_input_stabilizer(input_stabilizer_config) else {}
            ),
