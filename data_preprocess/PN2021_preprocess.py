...
def _validate_nonfinite_policy(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(policy, dict):
        raise ValueError("PN2021 config must define nonfinite_policy")
    if str(policy.get("repair_method")) != "linear_per_lead_nearest_edge":
        raise ValueError(
            "PN2021 nonfinite repair_method must be "
    ...
    for name, value in (
        ...
        if not 0.0 <= value <= 1.0:
            raise ValueError(f"PN2021 {name} must be in [0, 1], got {value}")
    return {
...
def _load_config(config_path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"PN2021 config must be a YAML mapping: {path}")
    return payload
    ...
    *,
    ...
    if int(round(source_fs)) == target_fs and abs(source_fs - target_fs) < 1e-6:
        ...
        if cropped.shape[0] == 1 or resized_samples == 1:
            ...
            resampled = _linear_interpolate_time_batch(
                cropped[None, ...],
                resized_samples,
            )[0]
