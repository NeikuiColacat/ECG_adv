...
by itself to identify the selected records. Consumers must use the frozen split
directory from `ecg_k500_data_models_handoff_20260731.tar.zst` and verify the
source-manifest, split-manifest, and hash-set identities exposed by
...
- `train/PTBXL.yaml`
- `train/vae.yaml`
- `baselines/ptbxl_source_v1.yaml`
...
Runtime caches, split arrays, and model weights remain outside Git. Only the
portable contract and SHA256 identities are tracked here.
