...
from ecg_adv_gen.training import SynthNPZDataset, compute_pos_weight, masked_bce_with_logits  # noqa: E402
...
def load_center_real_records(args) -> list[dict]:
    ...
    scanned = sorted(
        scan_pn2021_center_records(Path(args.pn2021_root) / "training" / args.center),
        key=lambda record: str(record.header_path),
    )
    if allowed is not None:
        scanned = [record for record in scanned if record.record_id in allowed]
    records = []
    for record in scanned:
        codes = list(record.snomeds)
        ...
        if proc is None:
            continue
        records.append({"record_id": record.record_id, "signal": proc, "label": y})
