...
def train_raw_corruption_consistency_epoch(
    ...
    *,
    ...
    raw_first_batches = 0
    forced_renorm_batches = 0

    for batch_i, batch in enumerate(loader, start=1):
        if len(batch) == 3:
            signals, labels, corruption_base = batch
            raw_first_batch = True
        else:
            signals, labels = batch
            corruption_base = signals
            raw_first_batch = False
        signals = signals.to(device, non_blocking=True)
        labels = labels.to(device, non_blocking=True)
        raw_first_batches += int(raw_first_batch)

        clean_np = corruption_base.detach().cpu().numpy().astype(np.float32, copy=False)
        effective_renorm = bool(renorm or raw_first_batch)
        forced_renorm_batches += int(raw_first_batch and not renorm)
        if view_mode == "augmix":
            corrupt_np, stats = build_raw_augmix_views(
                ...
                op_schedule=augmix_op_schedule,
                ops=ops,
                rng=rng,
                renorm=effective_renorm,
                clip_abs=clip_abs,
    ...
    result = {
        ...
        "ops": list(ops),
        "op_counts": op_counts,
        "renorm": bool(renorm or raw_first_batches > 0),
        "renorm_requested": bool(renorm),
        "renorm_forced_for_raw_first_batches": int(forced_renorm_batches),
        "input_space": "raw_pre_zscore" if raw_first_batches > 0 else "model_input",
...
def build_raw_corruption_views(
    ...
    *,
    ...
    """Build raw corruption views through the package-level view core."""
...
def build_raw_augmix_views(
    ...
    *,
    ...
    """Build full-pool raw AugMix views through the package-level view core."""
...
def parse_args():
    ...
    p.add_argument(
        ...
        default="",
        help="YAML/JSON operator profile file used only with --raw_corrupt_severity_profile custom.",
    )
    p.add_argument(
        "--raw_corrupt_severity_params_name",
        default="",
        help="Profile name under top-level profiles used only with --raw_corrupt_severity_profile custom.",
    )
    p.add_argument(
        "--raw_corrupt_ops",
        nargs="+",
...
def main():
    ...
    raw_corrupt_severity_profile_params = _resolve_optional_custom_severity_profile(
        enabled=bool(args.enable_raw_corrupt_consistency),
        severity_profile=args.raw_corrupt_severity_profile,
        params_file=args.raw_corrupt_severity_params_file,
        params_name=args.raw_corrupt_severity_params_name,
        flag_prefix="--raw_corrupt",
    )
    ...
    if args.enable_raw_corrupt_consistency:
        if args.raw_corrupt_scope == "target":
            ...
            raw_corrupt_ds = TargetRealRawFirstCorruptionDataset(
                real_signals,
                real_labels,
                crop_len=args.crop_len,
                mode="train",
        ...
        print(
            ...
            f"input_space={'raw_pre_zscore' if args.raw_corrupt_scope == 'target' else 'model_input'} "
            f"view_mode={args.raw_corrupt_view_mode} "
            f"augmix=(width={args.raw_augmix_width},depth={args.raw_augmix_depth},"
            f"alpha={args.raw_augmix_alpha},schedule={args.raw_augmix_op_schedule},"
            f"mixture={args.raw_augmix_mixture_mode}:"
    ...
    log: Dict[str, Any] = {
        ...
        "raw_corrupt_consistency": {
            ...
            "severity_params_file": str(args.raw_corrupt_severity_params_file),
            "severity_params_name": str(args.raw_corrupt_severity_params_name),
            ...
            "augmix_op_schedule": str(args.raw_augmix_op_schedule),
    ...
    for epoch in range(start_epoch, args.n_epochs + 1):
        ...
        if args.enable_raw_corrupt_consistency and raw_corrupt_consistency_loader is not None:
            raw_corrupt_stats = train_raw_corruption_consistency_epoch(
                ...
                severity_profile=args.raw_corrupt_severity_profile,
                severity_profile_params=raw_corrupt_severity_profile_params,
                ops=list(args.raw_corrupt_ops),
                ...
                augmix_op_schedule=args.raw_augmix_op_schedule,
