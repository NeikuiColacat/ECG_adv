...
from ecg_adv_gen.evaluation.views import (
...
VIEW_TO_CANONICAL = {
    "macro": PN2021C_ALL_ZERO_KEPT_CORRUPTED_REFEXCLUDED,
    "drop_all_zero": PN2021C_DROP_ALL_ZERO_CORRUPTED_REFEXCLUDED,
}
...
def _canonical_view(view: str) -> str:
    return VIEW_TO_CANONICAL[str(view)]
...
def _aggregate_rows(rows: list[dict[str, Any]], keys: tuple[str, ...]) -> list[dict[str, Any]]:
    ...
    for group_key, group_rows in sorted(groups.items()):
        ...
        if "view" in item:
            item["canonical_view"] = _canonical_view(str(item["view"]))
