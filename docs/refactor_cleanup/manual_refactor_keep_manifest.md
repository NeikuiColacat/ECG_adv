...
| `util/run_record.py` | `KEEP-MANUAL` | 记录命令、配置闭包、Git/dirty SHA和环境；校验 schema-v1 PN2021 train result、schema-v3 final checkpoint 身份与 schema-v3 evaluation subject，把 lineage/selection/path/SHA 写入 manifest/index | 新实现保留禁止 worktree 输出、原子 JSON、heldout-free last/eval 契约、三模式 subject、配置快照、完整索引及篡改检测；`--train-result` 不进 YAML closure 但必须由 subject 锁定 resolved path+SHA |
| `boot_scripts/run_experiment.py` | `KEEP-MANUAL` | 单一 experiment YAML + config-root 的白名单 launcher；四个 code-owned entrypoint 锁定脚本、primary result basename/type，dry-run零副作用；仅 `--method-config`/`--source-registry` 扩展配置闭包 | 新入口保留 PTB-XL 双骨干训练、PN2021 K500 训练与三模式评估的受限entrypoint/flags、配置闭包、外部唯一run dir、精确argv、预期结果身份和RunRecorder生命周期；`--train-result` 作为结果 artifact 不得混入 YAML closure |
...
| `util/tests/test_manual_run_experiment.py` | `KEEP-MANUAL` | 验证四入口统一 launcher 配置闭包、38 个 YAML 的 train/eval result 类型、dry-run/collision、真实执行 fail-closed、schema-v1 train/schema-v3 eval subject、run record/file index 和参数禁令 | 新 launcher 接管相同零副作用、输出防覆盖、白名单参数与完整性契约，并验证 `--method-config`/`--source-registry` 进闭包而 `--train-result` 仅由 subject 锁 path/SHA |
...
| `util/tests/test_pn2021_evaluation.py` | `KEEP-MANUAL` | 验证三种互斥 subject（前瞻 train-result、legacy A0/Direct、source registry）、wrong-center/schema/method fail-closed、schema-v3 result、corruption 锁、canonical4 顺序与 ECGFounder canonical100 链 | 新评测层接管相同 subject lineage、checkpoint/schema、corruption、中心聚合、ref-exclusion、CPSC 合并和模型输入契约 |
