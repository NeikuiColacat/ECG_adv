...
| 文件 | 状态 | 当前职责 | 删除或合并前必须满足 |
|---|---|---|---|
| `data_preprocess/preprocess_primitives.py` | `KEEP-MANUAL` | PTB-XL/PN2021 缓存 builder 共用的非有限值修复、路径、导联、WFDB、记录 hash、定宽字符串和 aligned-corner 线性插值唯一 owner | 新实现保持两 builder 的逐值输出、拒绝原因/详情与数据集错误文案完全等价 |
...
| `data_preprocess/augmentations_cache.py` | `KEEP-MANUAL` | 从白名单 PN2021 缓存和唯一 `AugmentationProfile` 生成五中心、depth2+3、100/500 Hz PN2021-C 连续缓存；CLI 仅保留实际构建，不维护未登记容量预览面 | 新入口保留源缓存身份、20组合、随机种子、双采样率契约、构建前磁盘余量闸门和单一严格 profile loader，不恢复 plan-only、dict fallback 或 snapshot wrapper |
...
| 文件 | 状态 | 当前职责 | 删除或合并前必须满足 |
|---|---|---|---|
| `util/augmentations/__init__.py` | `KEEP-MANUAL` | 空的显式包边界；调用方直接导入 `operators`、`profile` 或 `torch_operators` owner | 新包保持空导出与无 eager NumPy/Torch/YAML；不得恢复算子、profile 或 Torch alias re-export |
...
| `util/augmentations/torch_operators.py` | `KEEP-MANUAL` | 仅拥有在线 canonical corruption 使用的五个 device-native batched Torch kernel 与 prevalidated dispatcher；逐样本 NumPy-parity wrapper 已退出 | 新实现保留外层 raw-BTC/finite 校验、五算子参数拒绝、CPU/CUDA batch 数值、显式 generator/RNG 末态、非原地连续 float32 输出及仅限白名单调用的 prevalidated fast path；不恢复逐样本或 NumPy-style API |
...
| `util/config_bundle.py` | `KEEP-MANUAL` | 将 YAML 间引用限制在当前选中的 configs-shaped 配置束内，使完整配置副本可移植且不回落主配置；唯一拥有 concrete-dict gate、repo-relative YAML mapping loader 与受管训练/评估递归 YAML 配置闭包 | 新实现保留显式 config root、禁止相对路径逃逸、副本优先解析、严格 mapping 错误语义和闭包测试 |
...
| `models/efficientnet1d.py` | `KEEP-MANUAL` | 固定重写历史 EfficientNet1DV2 S-V2 五分类定义及 checkpoint 构建接口，并显式暴露冻结分类头前的 `forward_features` 供两链 AugMix-SimCLR 使用；不保留通用 EfficientNet variant/activation/norm 构造面 | 新实现与571个state key、旧checkpoint和100 Hz前向逐元素兼容，且 feature API 不改变原分类前向；factory 架构参数继续 fail closed |
| `models/ecgfounder.py` | `KEEP-MANUAL` | 固定重写官方12导联 ECGFounder Net1D 唯一受管拓扑、预训练backbone加载、Super5 head及 full/head trainable scope；不保留通用 width/depth/BN/dropout 构造面或内部 helper export，复用既有 `forward_features` 接入同一 Stage-1 | 新实现与509个官方state key、415个module path、30,670,389参数、官方checkpoint和500 Hz前向逐元素兼容，且两个骨干共享同一表征学习接口 |
...
| `models/vae.py` | `KEEP-MANUAL` | 固定 ECGTwin VAE 唯一受管 1-head/512 attention 拓扑、严格双 state-dict 构建与四函数 owner 面；encoder 仅返回确定性 `(scaled_mean, mean, log_variance)` 三元组，decoder helper 仅返回 canonical raw100 | 新实现保留 `(B,1024,12)` raw mV、scaled `(B,4,128)` posterior mean、logvar、导联交换、0.18215 scale、固定 `(B,1000,12)` 解码、240个 state key、175个 module path、29,552,112参数、配置拒绝语义与真实 checkpoint 前向；不恢复通用 attention 构造、VAE config 镜像、encoder 随机采样或 decoder 5000点分支 |
...
| `configs/train/methods/a1_corrupt_ft_rot4_v1.yaml` | `KEEP-MANUAL` | schema-v2 A1 监督 rotating4 腐蚀微调选择器；复用主线 Stage-2 corruption RNG 身份且不引用 AugMix、teacher 或 VAE | 新 selector 保留每批 `0.5 clean + 4×0.125 corruption`、每轮两 depth2+两 depth3、五轮覆盖20组合、family-balanced BN、完整 K500 与单次 outer step |
...
| `core/train_PTBXL.py` | `KEEP-MANUAL` | 唯一拥有两骨干 PTB-XL YAML profile→model kwargs/dry-run JSON/训练委托，并从同一配置构造 raw100 `PTBXLLoaderPlan`、folds 1-8/9/可选10 与 code-owned model input adapter；`train_ptbxl` 直接拥有 loader 打开/关闭生命周期，不保留私有 bundle/builder 镜像层 | 新 PTB-XL 适配器保留 split/cache/raw100/finite-plan 身份、三份 direct boot JSON、双配置读取/校验、EffNet/ECGFounder 精确构建/适配顺序、Founder 唯一 `epochs=10` override、未授权时不构造 fold10，并继续只依赖白名单数据层和通用训练器 |
...
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_ningbo.yaml` | `KEEP-MANUAL` | EfficientNet E25 Ningbo A0 clean-only matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_chapman_shaoxing.yaml` | `KEEP-MANUAL` | EfficientNet E25 Chapman-Shaoxing A0 clean-only matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_cpsc_2018.yaml` | `KEEP-MANUAL` | EfficientNet E25 CPSC 2018+Extra A0 clean-only matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_georgia.yaml` | `KEEP-MANUAL` | EfficientNet E25 Georgia A0 clean-only matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_ningbo.yaml` | `KEEP-MANUAL` | EfficientNet E25 Ningbo A1 supervised rotating4 matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_chapman_shaoxing.yaml` | `KEEP-MANUAL` | EfficientNet E25 Chapman-Shaoxing A1 supervised rotating4 matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_cpsc_2018.yaml` | `KEEP-MANUAL` | EfficientNet E25 CPSC 2018+Extra A1 supervised rotating4 matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_georgia.yaml` | `KEEP-MANUAL` | EfficientNet E25 Georgia A1 supervised rotating4 matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_ningbo.yaml` | `KEEP-MANUAL` | ECGFounder E30 Ningbo A0 clean-only matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_chapman_shaoxing.yaml` | `KEEP-MANUAL` | ECGFounder E30 Chapman-Shaoxing A0 clean-only matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_cpsc_2018.yaml` | `KEEP-MANUAL` | ECGFounder E30 CPSC 2018+Extra A0 clean-only matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_georgia.yaml` | `KEEP-MANUAL` | ECGFounder E30 Georgia A0 clean-only matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_ningbo.yaml` | `KEEP-MANUAL` | ECGFounder E30 Ningbo A1 supervised rotating4 matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_chapman_shaoxing.yaml` | `KEEP-MANUAL` | ECGFounder E30 Chapman-Shaoxing A1 supervised rotating4 matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_cpsc_2018.yaml` | `KEEP-MANUAL` | ECGFounder E30 CPSC 2018+Extra A1 supervised rotating4 matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_georgia.yaml` | `KEEP-MANUAL` | ECGFounder E30 Georgia A1 supervised rotating4 matched-base/update 训练 | 保留专用 A0/A1 训练配置、同一 source checkpoint、完整 K500、last checkpoint、唯一外部输出目录和可 dry-run 闭包 |
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_eval_ningbo.yaml` | `KEEP-MANUAL` | EfficientNet E25 A0 的 ref-excluded Ningbo Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_eval_chapman_shaoxing.yaml` | `KEEP-MANUAL` | EfficientNet E25 A0 的 ref-excluded Chapman-Shaoxing Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_eval_cpsc_2018.yaml` | `KEEP-MANUAL` | EfficientNet E25 A0 的 ref-excluded CPSC 2018+Extra Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_eval_georgia.yaml` | `KEEP-MANUAL` | EfficientNet E25 A0 的 ref-excluded Georgia Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_effnet_a0_e25_matrix.yaml` | `KEEP-MANUAL` | EfficientNet E25 A0 四中心 diagonal 聚合 | 保留 canonical 四中心顺序、exact-cohort matrix 配置、四个 evaluation result 路径和 artifact-only 聚合 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_eval_ningbo.yaml` | `KEEP-MANUAL` | EfficientNet E25 A1 rotating4 的 ref-excluded Ningbo Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_eval_chapman_shaoxing.yaml` | `KEEP-MANUAL` | EfficientNet E25 A1 rotating4 的 ref-excluded Chapman-Shaoxing Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_eval_cpsc_2018.yaml` | `KEEP-MANUAL` | EfficientNet E25 A1 rotating4 的 ref-excluded CPSC 2018+Extra Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_eval_georgia.yaml` | `KEEP-MANUAL` | EfficientNet E25 A1 rotating4 的 ref-excluded Georgia Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_effnet_a1_rot4_e25_matrix.yaml` | `KEEP-MANUAL` | EfficientNet E25 A1 rotating4 四中心 diagonal 聚合 | 保留 canonical 四中心顺序、exact-cohort matrix 配置、四个 evaluation result 路径和 artifact-only 聚合 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_eval_ningbo.yaml` | `KEEP-MANUAL` | ECGFounder E30 A0 的 ref-excluded Ningbo Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_eval_chapman_shaoxing.yaml` | `KEEP-MANUAL` | ECGFounder E30 A0 的 ref-excluded Chapman-Shaoxing Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_eval_cpsc_2018.yaml` | `KEEP-MANUAL` | ECGFounder E30 A0 的 ref-excluded CPSC 2018+Extra Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_eval_georgia.yaml` | `KEEP-MANUAL` | ECGFounder E30 A0 的 ref-excluded Georgia Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a0_e30_matrix.yaml` | `KEEP-MANUAL` | ECGFounder E30 A0 四中心 diagonal 聚合 | 保留 canonical 四中心顺序、exact-cohort matrix 配置、四个 evaluation result 路径和 artifact-only 聚合 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_eval_ningbo.yaml` | `KEEP-MANUAL` | ECGFounder E30 A1 rotating4 的 ref-excluded Ningbo Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_eval_chapman_shaoxing.yaml` | `KEEP-MANUAL` | ECGFounder E30 A1 rotating4 的 ref-excluded Chapman-Shaoxing Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_eval_cpsc_2018.yaml` | `KEEP-MANUAL` | ECGFounder E30 A1 rotating4 的 ref-excluded CPSC 2018+Extra Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_eval_georgia.yaml` | `KEEP-MANUAL` | ECGFounder E30 A1 rotating4 的 ref-excluded Georgia Clean/PN2021-C 评估 | 保留 prospective train-result + method-config lineage、drop-all-zero 主口径、20-view 与唯一输出目录 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_a1_rot4_e30_matrix.yaml` | `KEEP-MANUAL` | ECGFounder E30 A1 rotating4 四中心 diagonal 聚合 | 保留 canonical 四中心顺序、exact-cohort matrix 配置、四个 evaluation result 路径和 artifact-only 聚合 |
...
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_ningbo.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only ningbo 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_chapman_shaoxing.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only chapman_shaoxing 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_cpsc_2018.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only cpsc_2018 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_georgia.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only georgia 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_eval_ningbo.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only ningbo ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_eval_chapman_shaoxing.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only chapman_shaoxing ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_eval_cpsc_2018.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only cpsc_2018 ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_eval_georgia.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only georgia ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_augmix_only_matrix.yaml` | `KEEP-MANUAL` | EfficientNet AugMix-only 四中心 diagonal 聚合 | 保留 canonical 四中心顺序、exact-cohort 配置和四个唯一 evaluation result |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_ningbo.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only ningbo 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_chapman_shaoxing.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only chapman_shaoxing 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_cpsc_2018.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only cpsc_2018 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_georgia.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only georgia 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_eval_ningbo.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only ningbo ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_eval_chapman_shaoxing.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only chapman_shaoxing ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_eval_cpsc_2018.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only cpsc_2018 ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_eval_georgia.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only georgia ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_augmix_only_matrix.yaml` | `KEEP-MANUAL` | ECGFounder AugMix-only 四中心 diagonal 聚合 | 保留 canonical 四中心顺序、exact-cohort 配置和四个唯一 evaluation result |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_ningbo.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only ningbo 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_chapman_shaoxing.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only chapman_shaoxing 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_cpsc_2018.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only cpsc_2018 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_georgia.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only georgia 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_eval_ningbo.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only ningbo ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_eval_chapman_shaoxing.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only chapman_shaoxing ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_eval_cpsc_2018.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only cpsc_2018 ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_eval_georgia.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only georgia ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_effnet_vae_lhat_only_matrix.yaml` | `KEEP-MANUAL` | EfficientNet VAE-LHAT-only 四中心 diagonal 聚合 | 保留 canonical 四中心顺序、exact-cohort 配置和四个唯一 evaluation result |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_vae_lhat_only_ningbo.yaml` | `KEEP-MANUAL` | ECGFounder VAE-LHAT-only ningbo 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_vae_lhat_only_chapman_shaoxing.yaml` | `KEEP-MANUAL` | ECGFounder VAE-LHAT-only chapman_shaoxing 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_vae_lhat_only_cpsc_2018.yaml` | `KEEP-MANUAL` | ECGFounder VAE-LHAT-only cpsc_2018 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_vae_lhat_only_georgia.yaml` | `KEEP-MANUAL` | ECGFounder VAE-LHAT-only georgia 单种子 matched 组件消融训练 | 保留同源 checkpoint、完整 K500、固定 recipe、预算、last checkpoint 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_vae_lhat_only_eval_ningbo.yaml` | `KEEP-MANUAL` | ECGFounder VAE-LHAT-only ningbo ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
| `configs/experiments/manual_refactor_pn2021_ecgfounder_vae_lhat_only_eval_chapman_shaoxing.yaml` | `KEEP-MANUAL` | ECGFounder VAE-LHAT-only chapman_shaoxing ref-excluded Clean/PN2021-C 评估 | 保留 train-result+method-config lineage、drop-all-zero、20-view 与唯一外部输出 |
...
| `util/tests/test_torch_augmentations.py` | `KEEP-MANUAL` | 验证活动 Torch batch dispatcher 的五算子逐位输出/generator末态、copy/dependency/conditional 分支、Stage-1 两链身份与可选 CUDA device 契约 | 新设备 batch kernel 接管相同 live 输出、RNG、非原地与设备契约；不恢复已退役逐样本 wrapper 的接口/NumPy 适配测试 |
...
| 2026-08-13 | 批次 25（augmentation package boundary） | 无底层实现文件删除；`util/augmentations/__init__.py` 收为 docstring + 空 `__all__`，删除 16 项 NumPy/profile/Torch eager barrel re-export | cache builder 与三份测试分别直接导入 `operators`、`profile`、`torch_operators` owner；包导入不再加载 Torch 或暴露旧 re-export 属性，底层 operator/profile 实现保持不变 | 相对 `a5fd7bb` 净减 37 physical LOC（25 insertions / 62 deletions）；生产 Python净删 45 行、测试 Python净增 7 行、文档净增 1 行；focused 58 passed/10 skipped；fresh-process probe 锁定空导出、无旧属性及无 eager Torch/NumPy/YAML，旧 barrel 导入引用为零；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 40,601（runtime Python 24,520；test Python 6,609；config YAML 4,499；全部 tracked YAML 4,516） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch25 package boundary 收敛；本批不改 owner 实现、YAML、公式、RNG、数据或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 26（supervised finite boundary） | 无底层训练能力删除；复用 checkpoint SHA owner，固定内部 Super5 strict metric 参数，删除 PTB-XL boot 三个零调用透传、loader bundle 两个零调用描述面、三个重复 model-spec parser、恒 true recipe 属性与内部-only `__all__` 项 | 通用 `train_model` 继续保留 device/pos-weight/class-names、no-adapter、optional validation、last、AMP/scheduler/checkpoint/result schema；PTB-XL boot 只保留有限 plan/run/train 公共面 | 相对 `0c33212` 净减 54 physical LOC（45 insertions / 99 deletions）；生产 Python净删 66 行、测试 Python净增 11 行、文档净增 1 行；321 passed/10 skipped；三份 direct boot JSON 逐字节一致，16-case model-spec acceptance/error-class/message mismatch=0，last/validation result schema exact；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 40,547（runtime Python 24,454；test Python 6,620；config YAML 4,499；全部 tracked YAML 4,516） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch26 supervised low-risk subset；本批不改 YAML、公式、RNG、数据、优化、模型、checkpoint/result schema 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 27（preprocess primitive ownership） | 无缓存或数据删除；新增 `preprocess_primitives.py` 单一 owner，删除 PTB-XL/PN2021 builder 内两份重复的 YAML mapping、非有限值修复/policy、路径、导联、WFDB、记录 hash、定宽字符串与 aligned-corner 插值实现 | 两 builder 仅以 owner-qualified 调用复用 primitives；保留数据集专属 build loop、metadata、labels、QC reason/details、错误文案、100/500 Hz 数值与缓存 schema | 相对 `92882b5` 净减 85 physical LOC（320 insertions / 405 deletions）；生产 Python净删 143 行、测试 Python净增 55 行、config YAML净增 1 行、文档净增 2 行；focused 26 passed；独立 PTB-XL/PN2021 primitive corpus 的 22 类输出/异常逐项一致，旧重复定义退出受 AST 节点约束；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 40,462（runtime Python 24,311；test Python 6,675；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch27 single-owner 收敛；本批不改 build loop、metadata、labels、YAML 参数、数据/cache bytes、算法或 evidence，不新增测试文件 |
| 2026-08-13 | 批次 28（zero-consumer runtime surface retirement） | 无运行文件删除；删除 LHAT 零调用 generator、旧逐批 exact-label selector、独占 validator 与 float diagnostics wrapper；删除 latent pool 的 selection/cache attack wrapper、连带 resolver/map、冗余 ineligible cache/property 与 eligibility getter；删除 method runtime 两个零调用便利属性及位置 helper，并收掉 resident dataset 三个零调用 tensor/state getter | exact-label M20 继续由 schema-v2 stable neighbor table 预计算并只经 hash route 消费；保留 pool selection/cache tensors、唯一性校验与 identity SHA、显式不可用清单、`LHATDiagnostics.select`、全部 `LatentAttackBatch` 字段及 resident item/batch/describe/close 行为 | 相对 `c4dc610` 净减 221 physical LOC（26 insertions / 247 deletions）；生产 Python净删 244 行、测试 Python净增 22 行、文档净增 1 行；focused 102 passed，full 322 passed/10 skipped；现有两个测试节点以紧凑 negative assertions 锁定退役面及保留 identity 字段，生产/测试/YAML 旧符号引用为零；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 40,241（runtime Python 24,067；test Python 6,697；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch28 frozen zero-consumer deletion map；本批不改 YAML、邻居表/攻击/收缩算法、RNG、数据、模型、训练/评估 schema、`LatentAttackBatch` holder 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 29（cache-builder utility ownership） | 无运行文件删除；`split_cache.py` 与 `augmentations_cache.py` 删除两份 repo-relative path resolver 和两份文件 SHA helper，直接复用 config-bundle path owner 与 artifact-contract SHA owner；augmentation builder 再删除重复 Super5/导联常量，split builder 删除零消费者 `__all__` | 两 builder 保留各自 split/cache build loop、配置校验、manifest schema 与原子/no-clobber 行为；共享 artifact owner 以私有 module-qualified 方式调用，augmentation 比较和 manifest 写出显式把 owner tuple 转为 list，保持异常与 JSON 序列化 exact | 相对 `93deac2` 净减 26 physical LOC（40 insertions / 66 deletions）；生产 Python净删 42 行、测试 Python净增 15 行、文档净增 1 行；focused 25 passed，full 322 passed/10 skipped；路径/SHA probe 与 list/JSON 对照逐项一致，现有两个测试节点锁共享 owner 身份、旧 helper/常量定义及 split export 退出；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 40,215（runtime Python 24,025；test Python 6,712；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch29 frozen two-builder helper ownership scope；本批不改 YAML、split/cache 算法、RNG、数据/artifact bytes、manifest schema、活动 evidence 或其他 builder，不新增测试节点 |
| 2026-08-13 | 批次 30（runtime public holder surface slim） | 无运行文件删除；`TwoChainAugMixBatch` 公开 holder 从 14 字段缩为仅保留活动消费者读取的 `mixed_raw`；`CorruptionDiagnostics` 从十字段缩为四个逐样本 provenance tensor，删除六个固定默认元数据字段；`_quality_mask` 删除零调用的 `return_reasons=False` 分支 | 两个 dataclass、生成函数及其 `__all__` 继续保留；两链顺序、Dirichlet/Beta 混合、500 Hz 算子域、composition/depth/operator/nonfinite 诊断和始终返回 QC reason 的活动路径不变；本行明确覆盖并登记有意的 public holder schema 缩窄，不提供旧字段兼容属性 | 相对 `e7043f9` 净减 33 physical LOC（9 insertions / 42 deletions）；生产 Python净删 41 行、测试 Python净增 7 行、文档净增 1 行；focused 72 passed/10 skipped，full 322 passed/10 skipped；现有测试节点精确锁定两个 dataclass 字段集合与 quality signature；旧 holder 字段及 `return_reasons` 的生产/YAML 消费为零；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 40,182（runtime Python 23,984；test Python 6,719；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch30 frozen holder/runtime scope；本批不改 YAML、公式、RNG、数据、模型、训练/评估逻辑或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 31（online/latent internal holder slim） | 无运行文件删除；退役零消费者 method-config 目录常量及 export、`OnlineTrainConfig.profile_name` 便利属性、`OnlineTrainingResult` 的未序列化 `model/history` 镜像字段，以及 `LatentAttackBatch` 五个零读索引/hash 镜像字段与唯一物化代码；`LatentAttackBatch` 类与返回注解保留但从无消费者 `__all__` 退出 | config payload 的 `profile_name`、caller-owned model、`training_history.json`、checkpoint、`OnlineTrainingResult.describe()`/result schema、LatentPool selection/cache/identity 及 attack holder 四个活动 tensor 不变；本行明确覆盖 Batch28 对全部 attack holder 字段的阶段性保留，不提供兼容属性或 re-export | 相对 `c521295` 净减 17 physical LOC（9 insertions / 26 deletions）；生产 Python净删 26 行、测试 Python净增 8 行、文档净增 1 行；focused 38 passed；现有测试节点锁定退役常量/property、两类 dataclass 精确字段集合及 export 边界；旧 holder 字段的生产/YAML 消费为零；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 40,165（runtime Python 23,958；test Python 6,727；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch31 frozen closed-repo holder scope；本批不改 YAML、公式、RNG、数据、模型、训练/评估 artifact schema 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 32（finite S-V2 architecture specialization） | 无运行文件删除；`EfficientNet1DV2` 从通用 family 构造器收为唯一受管 S-V2 常量结构，删除18个非 `s_v2` variant、六种activation、四种norm、架构kwargs/分支与七个零消费者 helper/block export | 保留 canonical class/builder、checkpoint/map-location/strict、34-block模块树、共享激活对象、`CustomNorm.norm` 与 `classifier.3` state路径、初始化顺序、`forward_features`/head顺序和 stochastic-depth RNG；unknown architecture kwargs 自然 TypeError，不提供兼容层 | 相对 `ced4c15` 净减 147 physical LOC（66 insertions / 213 deletions）；生产 Python净删 165 行、测试 Python净增 17 行、文档净增 1 行；focused 87 passed；现有 data/model 节点锁定无参 class、class/builder 二符号 export、571 state key、6,413,827参数、687 module path、16/18 block分布与 factory architecture kw 拒绝；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 40,018（runtime Python 23,793；test Python 6,744；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch32 frozen finite-model scope；本批不改 YAML、checkpoint、forward/feature数值、RNG、训练/评估逻辑或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 33（finite mmap batch-access specialization） | 无数据或配置删除；`load_cache` 退役零外部消费者的 `ECGRecord/get_record`、scalar/NCT/layout/copy/unchecked 便利面，`data_runtime` 删除 `batch_read=false`、构造参数/holder 与逐条组装分支，scalar Dataset 访问统一委托批量路径 | 保留 cache manifest 的 storage-layout 校验、YAML `batch_read:true` 及其 SHA、owned contiguous finite raw BTC、`ECGBatch` 字段、PN2021-C view 解析、dense/sparse mmap、K500 resident、prefetch 与 sequential session；本行明确覆盖 Batch23A 对 batch-read/layout 的阶段性保留，不提供兼容层 | 相对 `80c8b45` 净减 130 physical LOC（43 insertions / 173 deletions）；生产 Python净删 153 行、测试 Python净增 22 行、文档净增 1 行；focused 45 passed；现有 data-runtime 节点锁定 `batch_read:false` 拒绝、九符号 cache surface、退役 helper/kwargs 与 batch-only signature；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 39,888（runtime Python 23,640；test Python 6,766；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch33 frozen finite-data scope；本批不改 YAML、split/cache/ledger bytes、采样率、导联/标签/映射、K500/ref-exclusion、RNG、训练/评估 schema 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 34（finite view observer surface slim） | 无运行文件删除；退役零消费者 `Provenance` class/export、`WaveformView.provenance` 与四份重复构造 payload，聚合前缀和 rejection node identity 直接复用等值 `WaveformView.name`；再删除六个零读 metadata 镜像键及仅为 anchor 镜像执行的 clone/index-copy | 保留 `WaveformView.name/metadata`、四个 accounting 键、diagnostic samples/weights、stochastic trace、quality rejection 与所有 recipe/history/result 输出；`core.lhat.LHATResult.anchor_waveform_raw`、contract endpoint residual、corruption diagnostics 计算、公式和 RNG 全部不动；本行明确登记 public holder/export 缩窄，不提供兼容属性或 re-export | 相对 `b2b4be8` 净减 78 physical LOC（14 insertions / 92 deletions）；生产 Python净删 90 行、测试 Python净增 11 行、文档净增 1 行；focused 38 passed；现有 method-graph 节点锁定 11符号 contracts export、九字段 WaveformView 与零读 metadata/anchor 镜像退出；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 39,810（runtime Python 23,550；test Python 6,777；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch34 frozen closed-repo observer scope；本批不改 YAML、recipe、loss/model/BN/RNG、数据、训练/评估 artifact schema 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 35（strict config/hash primitive ownership） | 无运行文件删除；12份 concrete-dict gate 与两份 repo-relative YAML mapping reader 收敛到 `util.config_bundle`，六份 Path-only 文件 SHA loop 收敛到纯 stdlib artifact owner；checkpoint 公共 SHA 保留 path expand/resolve 薄 wrapper | 保留原私有 alias 调用、mapping/YAML/SHA 输出与异常语义、checkpoint public surface；ledger 的 no-follow/fstat 防竞态哈希、artifact exact-key gate、JSON/ID/tensor/RNG/recipe payload 哈希明确不合并 | 相对 `711e51f` 净减 103 physical LOC（51 insertions / 154 deletions）；生产 Python净删 113 行、测试 Python净增 9 行、文档净增 1 行；focused 217 passed；现有三个测试节点锁 shared owner/export/alias 与 checkpoint wrapper，`py_compile` 与 `git diff --check` 通过。当前 tracked physical 39,707（runtime Python 23,437；test Python 6,786；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch35 two-owner exact consolidation；本批不改 YAML、数据、模型、公式、RNG、训练/评估 schema 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 36（finite ECGFounder architecture specialization） | 无运行文件删除；`ECGFounderNet1D` 从通用 width/depth/ratio/kernel/group/BN/dropout 构造器收为唯一受管官方常量拓扑，删除架构 kwargs、不可达分支、镜像 holder 与五个内部 helper export | 保留 canonical class/builder、509个state key、415个module path、30,670,389参数、全部 BN/Dropout 占位模块及构造顺序、`forward_features`/head、官方backbone与task checkpoint、full/head scope；unknown architecture kwargs 自然 TypeError，不提供兼容层 | 相对 `2af54c6` 净减 86 physical LOC（40 insertions / 126 deletions）；生产 Python净删 97 行、测试 Python净增 10 行、文档净增 1 行；现有 data/model 节点锁定无参 class、class/builder 二符号 export、509 state key、415 module path、参数量、stage depth与 factory architecture kw 拒绝；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 39,621（runtime Python 23,340；test Python 6,796；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch36 frozen finite-model scope；本批不改 YAML、checkpoint、500 Hz输入适配、forward/feature数值、RNG、训练/评估逻辑或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 37（PN2021-C build-only CLI） | 无数据或运行文件删除；退役零受管消费者的 `describe_cache_plan` 与 `--plan-only` 容量预览分支，CLI 收为 `--config` 后唯一委托真实 cache builder | 保留20组合、profile/seed/config身份、500→100 Hz数值与RNG、断点续建/no-overwrite、manifest schema和实际构建前磁盘余量闸门；明确登记旧 plan-only 命令不再兼容，不提供 wrapper | 相对 `dc39477` 净减 39 physical LOC（15 insertions / 54 deletions）；生产 Python净删 51 行、测试 Python净增 11 行、文档净增 1 行；现有 corruption 节点锁定普通 config 委托、plan symbol/flag退出；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 39,582（runtime Python 23,289；test Python 6,807；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch37 unregistered operational convenience retirement；本批不改 YAML、cache build loop/data/artifact bytes、训练/评估、metric或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 38（finite ECGTwin VAE boundary） | 无运行文件删除；`VAEConfig` 从十一字段缩为受管调用实际读取的 checkpoint path 与104/136 key counts，退役 component 上零读 config 镜像、通用 attention 构造参数及六个零消费者 owner export | 保留 package 五函数面、`load_vae_config`/builder/checkpoint override 签名与全部 YAML 拒绝语义；固定 1-head/512 attention、strict/frozen/eval，保持240个state key、175个module path、29,552,112参数、初始化RNG、真实 checkpoint encoder/decoder、0.18215 scale与双向 lead/length bridge逐位不变；不提供旧 holder/export 兼容层 | 相对 `65d0e0b` 净减 24 physical LOC（54 insertions / 78 deletions）；生产 Python净删 41 行、测试 Python净增 16 行、文档净增 1 行；现有 online-trainer 节点锁定三字段 config、四函数 owner export、两个内部无参 attention constructor、240-key/175-module/参数量与 config 镜像退出；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 39,558（runtime Python 23,248；test Python 6,823；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch38 frozen finite-VAE scope；本批不改 YAML、checkpoint/state/module路径、VAE/LHAT公式、RNG、数据、训练/评估 artifact schema 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 39（PTB-XL loader lifecycle ownership） | 无运行文件删除；退役仅被 `train_ptbxl` 消费、未导出的 `PTBXLDataLoaders` 与 `build_ptbxl_dataloaders`，将 train/validation/可选test 的打开、异常清理、训练委托与关闭生命周期并入唯一 owner | 保留三个公开 PTB-XL 入口、pure loader plan、双配置读取与 model/dataset/profile 校验、fold10 policy、打开及正序关闭、两骨干 input adapter、通用 supervised trainer 与 checkpoint/history/result schema；不提供私有 holder/helper 兼容层 | 相对 `cd46cae` 净减 29 physical LOC（83 insertions / 112 deletions）；生产 Python净删 33 行、测试 Python净增 3 行、文档净增 1 行；现有 PTB-XL 节点保持15函数/40 cases并改走真实 owner，新增 open-test/trainer 失败清理及两个退役符号断言；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 39,529（runtime Python 23,215；test Python 6,826；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch39 exact private lifecycle collapse；本批不改 YAML、split/cache/data、model、loss/optimizer/RNG、训练/评估 artifact schema 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 40（loader-owned finite objective boundary） | 无运行文件删除；退役 `MethodRequirements`、`ObjectiveTerm`、`ObjectivePlan` 三个中间 holder/export 及 `RecipeSpec.objective/.requirements`，objective term 与 VAE requirement 改由五个 code-owned finite definition 的 output identity 唯一派生；`RecipeSpec` 退出 public export并拒绝 direct constructor/`dataclasses.replace` 身份绕过 | 保留四个 tracked selector与唯一 matched-no-VAE slot、完整 describe/recipe SHA、惰性 objective view 生成、BCE `E/B`、family weights、pool/decoder独立拒绝、BN/RNG/history/checkpoint/result/evaluation schema；明确登记 public holder/direct-constructor schema 缩窄且不提供 compatibility property | 相对 `74d0878` 净减 16 physical LOC（178 insertions / 194 deletions）；生产 Python净删 57 行、测试 Python净增 40 行、文档净增 1 行；现有 method-graph 节点锁定五recipe objective/requirements/SHA、三 holder 与 constructor退出、matched-no-VAE身份和 lazy subset；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 39,513（runtime Python 23,158；test Python 6,866；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch40 finite objective single-source scope；本批不改 YAML、recipe/file identity、loss/optimizer/BN/RNG、数据、模型、训练/评估 artifact schema 或 evidence，不新增测试节点 |
| 2026-08-13 | 批次 41（live Torch batch-kernel boundary） | 无运行文件删除；退役 `torch_operators` 五个零生产消费者的逐样本/双形状 public wrapper、三项 scalar-only helper、Python-random 适配及旧 export，并同步删除只验证该退役面的测试矩阵 | `core.corruption` 继续唯一直接调用 prevalidated dispatcher，由其分派五个原样 batched kernel；保留 raw100→500 Hz→raw100、参数/RNG device拒绝、逐位输出与 generator 末态、copy/dependency/conditional、Stage-1 两链身份及可选 CUDA smoke；明确登记旧 wrapper/import API 不兼容且不提供别名 | 相对 `fbd3a40` 净减 636 physical LOC（178 insertions / 814 deletions）；生产 Python净删 398 行、测试 Python净删 239 行、文档净增 1 行；现有 Torch augmentation 节点收为16 passed/5 skipped的 live batch goldens，五个 wrapper 的 tracked production/YAML/launcher消费者为零，14个保留函数 AST exact、两处仅 docstring 更新；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 38,877（runtime Python 22,760；test Python 6,627；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch41 frozen live-kernel scope；本批不改 YAML、五个 batch kernel/dispatcher执行语句、公式、RNG、数据、模型、训练/评估 artifact schema 或 evidence，不新增测试文件节点 |
| 2026-08-13 | 批次 42（A5 owner export surface contraction） | 无运行文件删除；9个 A5 owner module 的 `__all__` 删除36个零 tracked production consumer 项，仅保留29个跨模块生产 direct-import export；全部符号定义、holder字段、函数签名、返回注解、同owner调用与显式导入能力保留 | 仅覆盖批次30对 `TwoChainAugMixBatch`/`CorruptionDiagnostics` 继续留在 `__all__` 的阶段性承诺，并延续批次31/40的 owner-defined-but-not-exported 边界；算法、公式、RNG、配置、数据/模型、训练/评估和 artifact schema不变；明确登记 wildcard/introspection public surface 收缩，不提供旧 star-export 保证 | 相对 `f02487f` 净减 16 physical LOC（20 insertions / 36 deletions）；生产 Python净删 36 行、测试 Python净增 19 行、文档净增 1 行；现有 method-graph 节点锁定9个 owner 的精确有序 export；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 38,861（runtime Python 22,724；test Python 6,646；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch42 closed-repo owner-export scope；本批不改 YAML、符号定义/构造签名、公式、RNG、数据、模型、训练/评估 artifact schema 或 evidence，不新增测试文件节点 |
| 2026-08-13 | 批次 43（finite recipe operation context specialization） | 无运行文件删除；退役仅由 `generate` 内两个固定 operation 消费的 `_RecipeContext`、generic resource map、per-context generator cache、`node_type` 与不可达 non-contract 分支；corruption/LHAT 直接绑定固定 node、资源和一次性 isolated generator | 保留四个 tracked selector与唯一 matched-no-VAE slot、recipe/describe SHA、`_derive_seed` payload、惰性 RNG 时机及 diagnostic key/value、波形/loss/BN/history/checkpoint/result/error schema；LHAT 仍由 loader 强制 exact-label attack-then-contract，不提供旧私有 context 兼容层 | 相对 `9823796` 净减 38 physical LOC（140 insertions / 178 deletions）；生产 Python净删 40 行、测试 Python净增 1 行、文档净增 1 行；现有 method-graph 节点锁定私有 context/分支退出，独立 A/B 锁定四recipe生成、describe、惰性RNG、11项错误及 contracted-LHAT payload；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 38,823（runtime Python 22,684；test Python 6,647；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch43 frozen finite-operation scope；本批不改 YAML、selector/spec identity、公式、RNG、数据、模型、训练/评估 artifact schema 或 evidence，不新增测试文件节点 |
| 2026-08-13 | 批次 44（PN2021 latent-pool lifecycle ownership） | 无运行文件删除；退役仅由 `train_pn2021` 消费的 `build_pn2021_latent_pool` 与单调用 `_encoder_sha256`，将 ordered K500 构池生命周期并入唯一训练 owner，并复用已解析的 recipe/config | 保留完整 K500 cache-index 顺序、encoder checkpoint SHA、M=20/standardizer epsilon、LHAT config 解析、ordered/training loader 双层关闭、encoder/decoder device 回迁、pool tensor/identity/RNG 与训练 history/checkpoint/result schema；本行仅覆盖批次42对 `build_pn2021_latent_pool` 定义及显式导入能力的阶段性保留，不提供私有 helper 兼容层；online trainer 自身仍独立解析 recipe/config 身份 | 相对 `4223d5e` 净减 24 physical LOC（92 insertions / 116 deletions）；生产 Python净删 32 行、测试 Python净增 7 行、文档净增 1 行；现有 online-trainer 节点改走真实 owner并锁定 VAE/no-VAE、ordered→pool→training 生命周期与构池参数；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 38,799（runtime Python 22,652；test Python 6,654；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch44 private lifecycle scope；本批不改 YAML、K500/split/cache/data、VAE/LHAT公式、RNG、模型、训练/评估 artifact schema 或 evidence，不新增测试文件节点 |
| 2026-08-13 | 批次 45（finite generated-batch carrier specialization） | 无运行文件删除；`GeneratedMethodBatch` 保留十字段 carrier，但退役唯一 code-owned producer 已结构保证的 generic `__post_init__` 复制/校验；diagnostic sample 与 stochastic trace 由每个唯一 view-prefix 的单 tensor 直接冻结，不再经 list + single-item `torch.cat` 中转 | 保留 generate accounting 遍历/排序/去重、全部 bundle/diagnostic/weight/sample/trace/position/count/rejection 字段、MappingProxy、detach 后独立 tensor storage、四recipe波形与 isolated RNG；明确登记非导出 carrier 的任意 direct-constructor 与伪造 metadata 错误面收窄，不提供泛化兼容层 | 相对 `eea5d84` 净减 35 physical LOC（15 insertions / 50 deletions）；生产 Python净删 38 行、测试 Python净增 2 行、文档净增 1 行；现有 method-graph 节点锁定 carrier 无 `__post_init__`，独立 A/B 锁定 A0/A3c/Direct/contracted-LHAT 完整 payload、mapping mutation、storage隔离与RNG；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 38,764（runtime Python 22,614；test Python 6,656；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch45 finite private-carrier scope；本批不改 YAML、recipe、loss/optimizer/BN/RNG、数据、模型、训练/评估 artifact schema 或 evidence，不新增测试文件节点 |
| 2026-08-13 | 批次 46（finite exposure transport specialization） | 无运行文件删除；退役唯一 production consumer 前的 `_iter_exposure_batches` 与六个 `__exposure_*`/composition/objective/loss-scale magic mapping 键，训练循环直接按 dataloader 外层、code-owned `_ExposureStep` 内层展开且复用同一 base tensor | 保留四种 finite exposure schedule、Direct21/rotating4+aux顺序、family loss scale、objective terms、RNG identity、BN plan、group finite gate、每base batch一次 zero-grad/optimizer step、history/checkpoint/result；明确登记 grouped batch 只接受受管 Mapping、不再兼容可被 `dict()` 转换的伪造输入或 magic-key 注入，不提供私有 helper兼容层 | 相对 `d290d41` 净减 47 physical LOC（18 insertions / 65 deletions）；生产 Python净删 41 行、测试 Python净删 7 行、文档净增 1 行；现有 online-trainer 节点锁定 transport与六magic-key退出，真实 Direct21 tiny train A/B 锁定history、model、optimizer、scheduler、checkpoint、stochastic trace与三套RNG；`py_compile` 与 `git diff --check` 通过。当前 tracked physical 38,717（runtime Python 22,573；test Python 6,649；config YAML 4,500；全部 tracked YAML 4,517） | 用户在 2026-08-13 的 12 小时精简 goal 中授权 Batch46 finite private-transport scope；本批不改 YAML、schedule、loss/optimizer/BN/RNG、数据、模型、训练/评估 artifact schema 或 evidence，不新增测试文件节点 |
