...
| `configs/data/data_load.yaml` | `KEEP-MANUAL` | 唯一数据运行配置；统一控制 RAM/mmap、K500 selection-resident、worker、pin-memory、prefetch，并以 SHA 声明 tracked content ledger | 新配置保留 split/cache/hash/顺序/RNG、ledger descriptor，并保证只 gather 已验证 selection、连续 CPU 存储、源 mmap 及时关闭、worker 安全门和 mmap 数值等价 |
