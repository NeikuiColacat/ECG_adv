...
| `configs/train/methods/exp_paired_augmix_latent_bridge_v1.yaml` | `KEEP-MANUAL` | schema-v2 latent-threechain RecipeSpec 选择器；为稳定路径原位保留，仅引用 VAE/AugMix 与隔离 RNG | 新 selector 保留 latent 身份和外部资源闭包；双 view、三链、BCE/JSD、质量交集、BN 权重及 encoder/decoder requirements 必须由代码所有；不得表述成 LHAT 或原 AugMix 精确复现 |
| `core/__init__.py` | `KEEP-MANUAL` | 通用监督训练、在线训练与五个有限 RecipeSpec 的唯一公共导出面 | 新核心包接管相同接口并完成训练调用迁移；禁止重新开放任意方法组合或动态注册 |
...
| `core/methods/__init__.py` | `KEEP-MANUAL` | 有限 RecipeSpec、直接 runtime 和小型 canonical view 合同的稳定公共导出面 | 新包保留显式静态 API，不暴露 DAG、任意 node、动态 import 或插件式方法组合 |
...
| `core/methods/registry.py` | `KEEP-MANUAL` | 严格加载五个 schema-v2 RecipeSpec selector，以代码白名单绑定 kind/variant/科学合同/objective/requirements/output/RNG 身份并生成路径无关 spec SHA | 新 loader 保留根键/recipe键/资源键精确检查、未知值与动态 import fail-closed、外部配置引用闭包和完整科学身份哈希 |
| `core/methods/runtime.py` | `KEEP-MANUAL` | 按有限 recipe 直接分派 canonical corruption、latent-threechain 与 attack-then-contract LHAT；只为候选充足且通过收缩/QC 的记录暴露辅助 view，其余 clean-only | 新 runtime 保留 K500 hash/标签绑定、exact-label pool、500 Hz 算子域、legacy RNG payload、contract accepted mask、组合拒绝原因、raw/contract 诊断、显式随机 trace 和无 agent_workspace import |
| `core/online_trainer.py` | `KEEP-MANUAL` | 执行五个 code-owned recipe；主线为冻结分类头的两链 AugMix-SimCLR、source/stage2 logit anchor、五轮 rotating4、VAE 辅助直接相加及一次 outer update；Stage-1 消费 runtime 已解析的同一 AugMixConfig | 新实现保留 pre-zscore raw mV、K500 teacher cache、两个骨干 feature API、legacy RNG、单步 family balance、辅助 BN/RNG snapshot-restore、optimizer/view计数、最终 checkpoint、无 heldout 选模和完整配置/seed闭包 |
| `core/train_PN2021.py` | `KEEP-MANUAL` | 通过 `data_runtime` 建立 raw 100 Hz 完整 K500 loader，校验 source checkpoint 锁，并按 RecipeSpec code-owned requirements 路由 latent pool、frozen encoder/decoder | 新适配层保留四逻辑中心、CPSC/Extra 合并、K500 partition 身份、source path/SHA 锁、resident workers 约束、无pool recipe资源隔离及 recipe 不进入 DataLoader seed |

### A6. 通用监督训练入口

| 文件 | 状态 | 当前职责 | 删除或合并前必须满足 |
|---|---|---|---|
| `configs/train/PTBXL.yaml` | `KEEP-MANUAL` | 通用训练器的 PTB-XL source profile；记录 folds 1-8/9/10、AdamW、BCE、AMP、fold9 macro-AUPRC 选模，以及 EffNet/ECGFounder boot 默认值和独立输出目录 | 新配置保留相同数据身份、模型输入、选模边界、模型初始化、配置束引用和 run-scoped JSON/checkpoint 路径 |
...
| `boot_scripts/train_pn2021.py` | `KEEP-MANUAL` | 从 PN2021 外层协议、显式 `--method-config` RecipeSpec selector/source checkpoint/model/center 启动完整 K500 训练；按 code-owned requirements 精确加载 VAE 组件，支持无副作用 dry-run | 新入口保留源 checkpoint 强制输入、同组 seed/full-FT/pos-weight、recipe 隔离、A0/A3c 零 VAE、主线严格 VAE 加载和全部显式覆盖记录；不接受 selection/refit 或任意组合分支 |
...
| `configs/eval/PN2021_matrix.yaml` | `KEEP-MANUAL` | 前瞻四中心 diagonal 聚合的最小配置，只锁 canonical 中心顺序、equal-view→equal-center 与结果文件名 | 新配置不得重新引入数据、模型、corruption 或 checkpoint 配置闭包 |
...
| `util/evaluation/metrics.py` | `KEEP-MANUAL` | 直接从 raw logits 计算 Super5 AUROC 与 sklearn Average Precision；显式 strict/skip-undefined、per-class/macro、kept/drop、中心/view 等权聚合 | 新实现禁止 sigmoid 饱和改变排序，保留 AP 非梯形 PR-AUC 定义、classes-used/正负计数、metric view、record/hash-label、depth2/3/23、equal-weight 与 canonical 四中心精确顺序语义 |
| `util/evaluation/pn2021.py` | `KEEP-MANUAL` | 固定评估raw100 clean+20-view PN2021-C；schema-v3 result 绑定 subject，并统一复用一个sequential session，不保留旧worker/runtime profile回退 | 新实现保留subject/model/中心一致性、checkpoint SHA、禁止PTB-XL shard、拒绝500 Hz cache、clean/corrupted同记录、显式view、record/hash-label顺序、loader/session关闭及完整输入链证据 |
| `util/evaluation/matrix.py` | `KEEP-MANUAL` | 从四个 schema-v3 prospective 单中心结果重算 diagonal clean/20-view 与 canonical 四中心等权汇总 | 新实现保留输入顺序/唯一性、共享 lineage、中心特异 K500 ref-exclusion、member SHA 和禁止读取成员 aggregate 的 fail-closed 契约 |
...
| `boot_scripts/aggregate_pn2021.py` | `KEEP-MANUAL` | 四个前瞻单中心 evaluation JSON 的纯 CPU/JSON diagonal 聚合薄入口；受管 launcher dry-run 不执行 delegate | 新入口保留 exactly-four `--result`、最小配置闭包、不构建或加载 data/model/GPU 和唯一外部输出 |
| `util/run_record.py` | `KEEP-MANUAL` | 记录命令、配置闭包、Git/dirty SHA和环境；校验 schema-v1 PN2021 train result、schema-v3 final checkpoint/evaluation及 schema-v1 diagonal matrix，把 lineage/selection/path/SHA 写入 manifest/index | 新实现保留禁止 worktree 输出、原子 JSON、heldout-free last/eval 契约、三模式 subject、四成员 matrix identity、配置快照、完整索引及篡改检测；`--train-result`/`--result` 不进 YAML closure但必须由结果 artifact 锁定 resolved path+SHA |
| `boot_scripts/run_experiment.py` | `KEEP-MANUAL` | 单一 experiment YAML + config-root 的白名单 launcher；五个 code-owned entrypoint 锁定脚本、primary result basename/type，dry-run零副作用；仅 `--method-config`/`--source-registry` 扩展配置闭包 | 新入口保留 PTB-XL 双骨干训练、PN2021 K500 训练、三模式评估与 diagonal 聚合的受限entrypoint/flags、配置闭包、外部唯一run dir、精确argv、预期结果身份和RunRecorder生命周期；`--train-result`/`--result` 作为结果 artifact 不得混入 YAML closure |
...
| `util/tests/test_method_graph.py` | `KEEP-MANUAL` | 验证五个有限 RecipeSpec 的精确 schema/resource closure、code-owned objective/requirements/scientific contract、路径无关 spec SHA、RNG golden 和动态 import 禁令 | 新 recipe 层接管相同有限集合、运行行为、资源身份、随机兼容与静态白名单契约 |
| `util/tests/test_online_trainer.py` | `KEEP-MANUAL` | 验证在线配置/参数闭包、两骨干 source checkpoint path/SHA 锁、五轮 rotating4、family-balanced BatchNorm，以及 schema-v1 train result 与 schema-v3 final checkpoint/history 的 exact lineage | 新 trainer 接管相同 source 身份、预算、调度、数据读取、BatchNorm、heldout-free last 和 lineage 契约 |
| `util/tests/test_manual_run_experiment.py` | `KEEP-MANUAL` | 验证五入口统一 launcher 配置闭包、40 个 YAML 的 train/eval/matrix result 类型、dry-run/collision、真实执行 fail-closed、schema-v1 train/matrix、schema-v3 eval subject、run record/file index 和参数禁令 | 新 launcher 接管相同零副作用、输出防覆盖、白名单参数与完整性契约，并验证 `--method-config`/`--source-registry` 进闭包而 `--train-result`/`--result` 仅由产物身份锁 path/SHA |
...
| `util/tests/test_pn2021_evaluation.py` | `KEEP-MANUAL` | 验证三种互斥 subject、wrong-center/schema/method fail-closed、schema-v3 result、corruption 锁、canonical4 顺序、ECGFounder canonical100 链，以及四个 prospective 单中心结果的 diagonal 重算/成员 SHA/共享 lineage/漂移拒绝 | 新评测与聚合层接管相同 subject lineage、checkpoint/schema、corruption、中心聚合、ref-exclusion、CPSC 合并、模型输入及纯 JSON matrix 契约 |
...
- [x] 将主线收敛为五个 code-owned finite RecipeSpec：YAML 只保留固定身份与
...
- [x] 将 `agent_workspace` 中锁定的精简开发 recipe 迁入白名单运行面：
  ...
  checkpoint/JSON diagnostics、VAE contract 诊断和配置/seed/resource 身份均已接管；
