...
| `core/train_PTBXL.py` | `KEEP-MANUAL` | 仅接受两种受管 `ModelSpec`，从 YAML 构造 raw100 `PTBXLLoaderPlan` 建立 folds 1-8/9；仅在 YAML 显式最终测试时构建 fold10，并以 code-owned adapter 在设备上转换模型输入后委托通用 trainer | 新 PTB-XL 适配器保留 split/cache/raw100/finite-plan 身份、EffNet/ECGFounder 精确适配顺序、未授权时不构造 fold10，并继续只依赖白名单数据层和通用训练器 |
| `boot_scripts/__init__.py` | `KEEP-MANUAL` | PTB-XL 薄启动脚本包边界 | 新启动包完整接管两个模型入口且不承载训练业务逻辑 |
| `boot_scripts/train_ptbxl_effnet.py` | `KEEP-MANUAL` | 从 PTBXL YAML 构建 EfficientNet1DV2并调用 `train_ptbxl`；CLI仅保留 launcher-owned config/output/dry-run 有限面 | 新入口保留 raw100→EffNet 适配、配置束、随机种子和输出身份，禁止恢复 checkpoint/device/DataLoader 超参旁路 |
| `boot_scripts/train_ptbxl_ecgfounder.py` | `KEEP-MANUAL` | 从 PTBXL YAML 加载官方 ECGFounder backbone/full scope并调用 `train_ptbxl`；除 launcher-owned 字段外仅保留受约束 `--epochs=10` sensitivity 面 | 新入口保留 raw100→device linear5000 输入桥、官方 checkpoint、full scope、配置束、随机种子和输出身份，禁止恢复任意 runtime override |
...
| `util/run_record.py` | `KEEP-MANUAL` | 记录命令、不可变配置闭包、data-ledger 双快照、Git/dirty SHA和环境；复用统一 validator 校验 schema-v1 PN2021 train、schema-v3 checkpoint/evaluation及 schema-v1 diagonal matrix | 新实现保留禁止 worktree 输出、原子 JSON、heldout-free last/eval、三模式 subject、四成员 matrix/config-owned cohort、派生指标复算、配置/ledger 快照与复哈、完整索引及实际 artifact 字节篡改检测 |
...
| `util/tests/test_train_ptbxl.py` | `KEEP-MANUAL` | 验证两骨干从 YAML 派生同一 raw100 `PTBXLLoaderPlan`、官方 fold 开口、fold10 holdback、loader ownership、EffNet/ECGFounder 精确设备输入适配及有限 boot CLI | 新 PTB-XL 适配层接管相同 raw BTC、`1000→5000 align_corners=True`、先插值后 z-score、配置单一来源、关闭语义及无 runtime override 契约 |
