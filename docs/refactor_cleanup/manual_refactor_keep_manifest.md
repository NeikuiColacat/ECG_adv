...
| `util/pn2021_artifact_contract.py` | `KEEP-MANUAL` | 纯 stdlib 的 PN2021 lineage/train/evaluation/checkpoint/source/legacy artifact validator；统一解析 path/SHA 并复哈 checkpoint bytes | 新实现保留 exact schema、三模式隔离、heldout-free selection、K500/ref-exclusion、config-owned cohort 与无 Torch 导入；不得以宽松 truthy 字段或声明 SHA 代替字节验证 |
...
| `util/evaluation/matrix.py` | `KEEP-MANUAL` | 从四个 schema-v3 prospective 单中心结果重算 diagonal clean/20-view 与 canonical 四中心等权汇总 | 新实现保留输入顺序/唯一性、config-owned cohort、中心特异 K500/ref-exclusion、成员与实际 checkpoint 字节 SHA，以及成员 aggregate 复算校验；不得导入 Torch 或反序列化权重 |
...
| `util/run_record.py` | `KEEP-MANUAL` | 记录命令、配置闭包、Git/dirty SHA和环境；复用统一 validator 校验 schema-v1 PN2021 train、schema-v3 checkpoint/evaluation及 schema-v1 diagonal matrix，把 lineage/selection/path/SHA 写入 manifest/index | 新实现保留禁止 worktree 输出、原子 JSON、heldout-free last/eval、三模式 subject、四成员 matrix/config-owned cohort、派生指标复算、配置快照、完整索引及实际 artifact 字节篡改检测；`--train-result`/`--result` 不进 YAML closure但由结果 artifact 锁 path/SHA |
...
| `util/tests/test_data_runtime.py` | `KEEP-MANUAL` | 验证唯一 data-load 配置、augmentation→sanitize→z-score→layout 顺序、非有限值/shape fail-closed、selection 泄漏与 mmap prefetch；并仅以临时小 fixture 锁 canonical ledger、迁移等价、missing/extra/size/same-size drift、symlink/no-clobber/TOCTOU 边界 | 新运行时/账本层接管相同配置、数值顺序、泄漏、有界预取，以及 quick inventory-only/full content 分级和扫描稳定性契约 |
...
| `util/tests/test_manual_run_experiment.py` | `KEEP-MANUAL` | 验证五入口统一 launcher 配置闭包、40 个 YAML 的 result 注册、dry-run/collision、schema-v1 train result、run record/file index 和参数禁令；评估/matrix seal 不再以假 metric fixture 重复测试 | 新 launcher 接管相同零副作用、输出防覆盖、白名单参数与完整性契约，并验证 `--method-config`/`--source-registry` 进闭包而 `--train-result`/`--result` 仅由产物身份锁 path/SHA |
...
| `util/tests/test_pn2021_evaluation.py` | `KEEP-MANUAL` | 验证三种互斥 subject、legacy/source bypass 拒绝、schema-v3 result、corruption 锁、canonical4、ECGFounder canonical100、真实 evaluator→recorder seal、派生指标篡改、checkpoint 字节复哈、config-owned cohort 与 artifact-only import 隔离 | 新评测与聚合层接管相同 subject lineage、checkpoint/schema、corruption、中心聚合、ref-exclusion、CPSC 合并、模型输入和无 Torch matrix import 契约 |
